...
Future<void> sharePayrollPdf(BuildContext context, PayrollResult result, {bool annual = false}) => _share(
  context,
  'bilans-pay-${result.system.countryCode.toLowerCase()}-${_stamp(DateTime.now())}.pdf',
  (kit) => buildPayrollPdf(kit, result, annual: annual),
);
  ...
  doc.addPage(
    pw.MultiPage(
      ...
      header: (ctx) => kit.header(
        ctx,
        title: l.payTitle,
        subtitle: '${l.systemName(r.system)} · ${annual ? l.commonAnnual : l.commonMonthly} · ${l.payRulesFrom(rulesDate)}',
        date: date,
      ),
...
Future<Uint8List> buildTeamPdf(ReportKit kit, List<TeamMember> members, {bool annual = false, DateTime? now}) async {
  ...
  doc.addPage(
    pw.MultiPage(
      ...
      margin: ReportKit.margin,
      header: (ctx) =>
          kit.header(ctx, title: l.teamTitle, subtitle: '${l.teamMembers(members.length)} · ${annual ? l.commonAnnual : l.commonMonthly}', date: date),
      footer: (ctx) => kit.footer(ctx),
      build: (ctx) => [
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(l.pdfLanguageTitle, style: t.titleLarge),
            ),
            ...
            if (qr?.payload != null) ...[
              ...
              pw.Column(
                children: [
                  ...
                  pw.SizedBox(
                    width: 30 * PdfPageFormat.mm,
                    child: pw.Text(t((x) => x.invQrCaption), style: kit.small, textAlign: pw.TextAlign.center),
                  ),
                ],
              ),
            ],
